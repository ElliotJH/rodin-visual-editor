...
import org.eventb.emf.core.diagram.edit.commands.MachineRefinesCreateCommand;
import org.eventb.emf.core.diagram.edit.commands.MachineRefinesReorientCommand;
...
public class MachineItemSemanticEditPolicy extends
	...
	public MachineItemSemanticEditPolicy() {
		super(EventbcoreElementTypes.Machine_2002);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (EventbcoreVisualIDRegistry.getVisualID(incomingLink) == MachineRefinesEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (EventbcoreVisualIDRegistry.getVisualID(outgoingLink) == MachineSeesEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
			...
			if (EventbcoreVisualIDRegistry.getVisualID(outgoingLink) == MachineRefinesEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (EventbcoreElementTypes.MachineRefines_4005 == req.getElementType()) {
